...
rm_interfaces::msg::GimbalCmd Solver::solve(const rm_interfaces::msg::Green &green_msg) {
    ...
    double distance_true = sqrt(green_msg.pose.position.x*green_msg.pose.position.x+green_msg.pose.position.y*green_msg.pose.position.y+green_msg.pose.position.z*green_msg.pose.position.z);
    double green_x = green_msg.pose_pixel.position.x;

    // Initialize gimbal_cmd
    rm_interfaces::msg::GimbalCmd gimbal_cmd; 
    gimbal_cmd.distance = distance_true;
    gimbal_cmd.yaw = green_x - 736.51017;
